...
void mexFunction(int plhs, mxArray *alhs[], int prhs, mxArray *arhs[])
{
...
/*int main(void)
{
	...
	for (size_t i = 0; i < nz; i++)
	{
		...
		{
			fscanf_s(pha, "%lf", &a[j*nz + i]);
			fscanf_s(phb, "%lf", &b[j*nz + i]);
		}
	}
	fclose(pha);
	fclose(phb);
	double *data = (double*)malloc(sizeof(double)*nx*nt);
	for (size_t i = 0; i < nx; i++)
	{
		for (size_t j = 0; j < nt; j++)
		{
			fscanf_s(phdata, "%lf", &data[j*nx + i]);
		}
	}
	fclose(phdata);
	double boundary[] = { 4.4385826077685947e-01,
		...
		9.9104037877288398e-01, 9.9775252935263015e-01,
		1.0000000000000000e+00 };
	double *M = (double*)malloc(sizeof(double)*nz*nx);
	rtm2d_fm2d(a, b, data, boundary, M, nz, nx, nt, 0.02,10.0);
	FILE *ph;
	fopen_s(&ph, "M", "w");
