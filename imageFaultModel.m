...
velocityFile = fullfile(rootd,'faultModelData','faultVelocityModel.segy');
...
% E.g.
%  remoteVelocityFile = '/home/user/faultModelData/faultVelocityModel.segy';
remoteVelocityFile = '';

shotFiles = fullfile(rootd,'faultModelData','faultShotRecords.segy');
% If the Shot Records file is not on a shared drive with the cluster, then
% specify the remote file location and see ATTENTION #2 below.
% E.g.
%  remoteShotFiles = '/home/user/faultModelData/faultShotRecords.segy';
remoteShotFiles = '';
...
units = {'m','s'};
% velocity model
dz = 10;            % depth spacing (m)
dx = 10;            % surface spacing (m)

% shot records
ns = 100;           % number of shots
ds = 10;            % shot spacing (m)
dxr = 10;           % receiver spacing (m)
xr  = 100;          % maximum receiver offset (m)
