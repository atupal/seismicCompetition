...
__global__ void getAB(double *v, double *a, double *b,double *M,double *data,
        double *fdm1,double *fdm2,double *fdm3,double dt,double dx, int nz, int nx,int nt)
{
	...
	double *boundary, double * snapshot, double *data, int nz, int nx, int nt,int bz,int it)
{
	...
	for (int ix = idx; ix < nx; ix+=idxDim)
	{
		for (int iz = idz; iz < nz; iz+=idzDim)
		{
            ...
            if(ix>=nx-20&&iz<bz)
            {
                fdm1[ix*nz + iz] = boundary[nx-ix-1] * fdm1[ ix *nz + iz];
                fdm2[ix* nz + iz] = boundary[nx-ix-1] * fdm2[ix *nz + iz];
            }
...
__global__ void gpuCopy_rtm2d(double *fdm1,double *fdm2, double *fdm3, int nz,int nx)
{
	int idx = threadIdx.x + blockIdx.x*blockDim.x;
	int idxDim = blockDim.x*gridDim.x;
	int idz = threadIdx.y + blockIdx.y*blockDim.y;
...
void rtm2d_fm2d(double *v, double *data, double *boundary, double* M, int nz, int nx, int nt, double dt, double dx)
{
	...
	getAB <<<numBlocks, threadsPerBlock >>>(dv, da, db,dM,ddata,dfdm1,dfdm2,dfdm3, dt, dx, nz, nx,nt);
    ...
    gpuCopy_rtm2d<<<numBlocks,threadsPerBlock>>>(dfdm1,dfdm2,dfdm3,nz,nx);
